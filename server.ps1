...
$GeminiModel    = if ($envBag["GEMINI_MODEL"]) { $envBag["GEMINI_MODEL"] } else { "gemini-2.5-flash-lite" }
...
function Save-Sops {
    ...
    # No-BOM UTF-8: [System.Text.Encoding]::UTF8 emits a BOM in PS 5.1, which
    # would corrupt sops.json for Vercel's JSON.parse. See CLAUDE.md PS trap #2.
...
TOPIC SILENCE RULE (do not fire on absence of discussion):
If a SOP's topic was NEVER raised in the transcript - neither the Care Specialist asked about it nor the patient brought it up - the rule does NOT fire, even when the rule's trigger_logic technically defaults to true on missing data (e.g. GEN-001 "dental visit > 6 months OR pending dental work" sounds like it should fire whenever dental is unknown, but it MUST NOT fire when dental was never discussed at all). Silence is missing data, not confirmed evidence. The correct routing for unknown topics is the follow-up questionnaire path (downstream post-process handles this), not a defensive fire.

Specifically: GEN-001 requires the transcript to actually mention dental, dentist, teeth, or pending dental work. JNT-001 requires the transcript to mention smoking/tobacco/cigarettes/vape. JNT-002 requires PT/therapy/rehab to be discussed. JNT-003 requires HbA1c/A1c/diabetes/blood sugar. JNT-004 requires opioids/pain medication. BAR-001 requires prior weight-loss surgery mention. BAR-002 requires EGD/endoscopy mention. BAR-003 requires dietitian/dietician/nutrition mention. If the keyword family for a rule is entirely absent from the transcript, do not fire the rule - the data must be collected via follow-up.
